<template>
  <div id="modal-comp">
    <general-button class="outerButton" @click.native="showModal = true"
        ...
        <transition name="fade" appear>
          <div class="preInterview">
            ...
            <div class="footerButton">
              ...
              >
              <general-button @click.native="updateInterview">
                Update Interview</general-button
              >
            </div>
          </div>
        </transition>
...
import Dropdown from "@/components/GeneralDropdown.vue";
import interviewApi from "@/api/interviewApi.js";
export default {
  components: { GeneralButton, Dropdown },
  data() {
    return {
      showModal: false,
      learningObjectives: [
...
\n\n10. I’m trying to understand this from more perspectives. Do you know 1 or 2 other people dealing with this problem who I can speak with?",
  ...
  methods: {
    async updateInterview() {
      if (
        this.currentInterviewGoals == null &&
        this.currentLearningObjectives == null
      ) {
        this.$store.commit("setTypeToast", "Error");
        this.$store.commit(
          "setMessage",
          "Please define the new goal or the new learning objectives before proceeding!"
        );
        ...
        let updateObjectivesGoal =
          await interviewApi.updateLearningObjectivesGoal(
            this.$route.params.id,
            updatePayload
          );
        if (updateObjectivesGoal.data.success) {
          this.$store.commit("setTypeToast", "Success");
          this.$store.commit(
            "setMessage",
            "Learning Objectives and Goal Updated!"
          );
          this.$store.commit("showToast");
          this.$router.go();
        }
        else{
            throw new Error("Could not update the Interview Learning Objectives and Goal")
        }
      }
    },
    passOption(value) {
